...
- All traffic can coexist in a single queue, with minimal computation and no
  flow awareness in network hops ([security](#security) considerations aside).
...
Fields required for **output** from the network, appended by each hop, for
reflection back to the sender:
...
data immediately in RTT 1, like IW does, implementations may do so (and this
**may** be absorbed by any capacity slack), but it's safest to increase the rate
only after capacity becomes available.
